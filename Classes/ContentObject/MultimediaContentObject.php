<?php
namespace C3\Tibs\ContentObject;
    ...
    public function render($conf = [])
    {
        ...
        if ($incFile) {
            ...
            if (\TYPO3\CMS\Core\Utility\GeneralUtility::inList('txt,html,htm', $fileinfo['fileext'])) {
                ...
                $parArray = [];
                // Src is added
                $width = isset($conf['width.']) ? $this->cObj->stdWrap(
                    $conf['width'],
                    $conf['width.']
                ) : $conf['width'];
                if (!$width) {
                ...
                $height = isset($conf['height.']) ? $this->cObj->stdWrap(
                    $conf['height'],
                    $conf['height.']
                ) : $conf['height'];
                if (!$height) {
                ...
                // fetching params
                $params = isset($conf['params.']) ? $this->cObj->stdWrap(
                    $conf['params'],
                    $conf['params.']
                ) : $conf['params'];
                ...
                if ($fileinfo['fileext'] == 'class') {
                    ...
                    switch (strtolower($fileinfo['fileext'])) {
                        case 'mp4':
                            $type = 'video/' . $fileinfo['fileext'];
                            $src = $parArray['src'];
                            unset($parArray['src']);
                            $content = '<video controls  ' . implode(' ', $parArray) . '>';
                            $content .= ' <source type="' . $type . '" ' . $src . '>';
